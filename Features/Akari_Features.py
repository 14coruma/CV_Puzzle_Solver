...
def get_digit(cell, ocr_model, allow_zeros = True, debug=False):
    ...
    (h, w) = cell.shape
    # Invert the image (since akari digit cells have black bg)
    cell = 255-cell
    cell = cv.threshold(cell, 0, 255, cv.THRESH_BINARY_INV | cv.THRESH_OTSU)[1]
    # if much of the mask is filled, then must be a black cell. Keep track of this for later
    black = -1
    percentFilled = np.count_nonzero(cell == 0) / float(w * h)
    if percentFilled > .3: black = 9 
    # Get rid of border, as best possible
    ...
    if percentFilled < 0.03: return black
    elif percentFilled > 0.6: return black
...
def construct_board(img, ocr_model, allow_zeros=True, debug=False):
    ...
    h_cell, w_cell = height//10, width//10
    # Blurr image to reduce noise in edges
    img = cv.GaussianBlur(img, (3,3), cv.BORDER_REFLECT)
    for i in range(100):
        y, x = h_cell*(i//10), w_cell*(i%10)
        cell = img[y:y+h_cell, x:x+w_cell]
        board[i//10, i%10] = get_digit(cell, ocr_model, allow_zeros, debug)
    return board
...
def visualize_board(img, board):
    ...
    for i in range(100):
        ...
        textX += x
        textY += y
        cv.putText(img, str(digit), (textX, textY), cv.FONT_HERSHEY_SIMPLEX, h_cell/45, (255, 0, 0), 2)
        # END: Code adapted from https://www.pyimagesearch.com/2020/08/10/opencv-sudoku-solver-and-ocr/
    return img

if __name__ == "__main__":
    filename, debug = "Images/akari.jpg", False
    if len(sys.argv) > 1: filename = sys.argv[1]
    img = cv.imread(filename, 0)
    ocr_model = models.load_model('Models/OCR_CNN_Trained')
    cropped = locate_puzzle(img, debug)
    board = construct_board(cropped, ocr_model, allow_zeros=True, debug=debug)
    print(board)
